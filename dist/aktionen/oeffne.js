...
const basisPfadExterneDateien = path.join(os.homedir(), 'git-repositories/content/informatik-studium');
export function öffneDurchBibtex(referenz) {
...
export function öffneDurchStichwort(stichwort) {
    const aufgaben = gibStichwortVerzeichnis().gibAufgabenMitStichwort(stichwort);
    if (aufgaben.size === 0) {
        console.log(`Das Stichwort ${stichwort} gibt es nicht. War ${gibStichwortBaum().findeÄhnliches(stichwort)} gemeint?`);
    }
    else {
...
export function öffne(referenz) {
...
export function öffneDurchGlobInVSCode(globMuster, cmdObj) {
